...
resource "aws_lb_target_group" "apim_backend_tg" {
  name = "apim-backend-tg"; port = var.apim_backend_port; protocol = "HTTP"; vpc_id = var.vpc_id; target_type = "ip"
  health_check { enabled = true; path = "/health"; matcher = "200-299" }
}
resource "aws_lb_listener" "apim_backend_listener" {
  load_balancer_arn = aws_lb.apim_backend_alb.arn; port = 80; protocol = "HTTP"
  default_action { type = "forward"; target_group_arn = aws_lb_target_group.apim_backend_tg.arn }
}
